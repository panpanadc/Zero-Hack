...
import cum.xiaomao.zerohack.util.graphics.GLObject
...
open class Shader(vertShaderPath: String, fragShaderPath: String) : GLObject, Helper {
    ...
    init {
        ...
        if (linked == 0) {
            ZeroHackMod.logger.error(glGetProgramInfoLog(id, 1024))
            glDeleteProgram(id)
    ...
    private fun createShader(path: String, shaderType: Int): Int {
        ...
        if (compiled == 0) {
            ZeroHackMod.logger.error(glGetShaderInfoLog(id, 1024))
            glDeleteShader(id)
