package cum.xiaomao.zerohack.gui.rgui
...
import cum.xiaomao.zerohack.util.delegate.FrameValue
...
open class Component(
    ...
    protected var relativePosX by setting("Pos X", posXIn, -69420.911f..69420.911f, 0.1f, { false },
        { _, it -> if (this is WindowComponent && ZeroHackMod.ready) absToRelativeX(relativeToAbsX(it).coerceIn(0.0f, max(scaledDisplayWidth - width, 0.0f))) else it })
    protected var relativePosY by setting("Pos Y", posYIn, -69420.911f..69420.911f, 0.1f, { false },
        { _, it -> if (this is WindowComponent && ZeroHackMod.ready) absToRelativeY(relativeToAbsY(it).coerceIn(0.0f, max(scaledDisplayHeight - height, 0.0f))) else it })
