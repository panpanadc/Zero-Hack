...
import cum.xiaomao.zerohack.util.ConnectionUtils
...
object UUIDManager : Manager() {
    private val file = File("${ZeroHackMod.DIRECTORY}/uuid_cache.txt")
    ...
    private fun requestProfile(nameOrUUID: String): PlayerProfile? {
        ...
        return if (response.isNullOrBlank()) {
            ZeroHackMod.logger.error("Response is null or blank, internet might be down")
            null
        } else {
            try {
                ...
                ZeroHackMod.logger.error("Failed parsing profile", e)
                null
            }
        }
    }
    ...
    private fun request(url: String): String? {
        return ConnectionUtils.requestRawJsonFrom(url) {
            ZeroHackMod.logger.error("Failed requesting from Mojang API", it)
        }
    }
    ...
    fun load(): Boolean {
        ...
        return try {
            ...
            ZeroHackMod.logger.info("UUID cache loaded")
            true
        } catch (e: Exception) {
            ZeroHackMod.logger.warn("Failed loading UUID cache", e)
            false
        }
    }
    ...
    fun save(): Boolean {
        return try {
            ...
            ZeroHackMod.logger.info("UUID cache saved")
            true
        } catch (e: Exception) {
            ZeroHackMod.logger.warn("Failed saving UUID cache", e)
            false
        }
    }
