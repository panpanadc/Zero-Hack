...
import cum.xiaomao.zerohack.util.graphics.texture.MipmapTexture
import cum.xiaomao.zerohack.util.readText
...
object EmojiManager : Manager() {
    private val localFile = File("${ZeroHackMod.DIRECTORY}/emoji.zip")
    ...
    private val job = defaultScope.launch(Dispatchers.IO) {
        try {
            ...
            ZeroHackMod.logger.warn("Failed to check emoji update", e)
        }

        try {
            zipCache = ZipCache(ZipFile(localFile))
            ZeroHackMod.logger.info("Emoji Initialized")
        } catch (e: Exception) {
            ZeroHackMod.logger.warn("Failed to load emojis", e)
        }
    }
    ...
    private fun streamToJson(stream: InputStream): JsonObject? {
        return try {
            ...
            ZeroHackMod.logger.warn("Failed to parse emoji version Json", e)
            null
        }
    }
    ...
    private fun loadEmoji(name: String) {
        ...
        try {
            ...
            ZeroHackMod.logger.warn("Failed to load emoji", e)
        }
    }
