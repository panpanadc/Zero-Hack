...
import cum.xiaomao.zerohack.module.modules.player.WebBurrow
...
internal object AutoWebBurrow : Module(
    name = "AutoWebBurrow",
    ...
    init {
        ...
        safeConcurrentListener<RunGameLoopEvent.Tick> {
            if (toggleTimer.tick(toggleDelay) && canBurrow() && shouldBurrow()) {
                WebBurrow.override = player.betterPosition
                WebBurrow.enable()
                if (notification) Notification.send(AutoWebBurrow, "$chatName triggered", 1000L)
