...
import cum.xiaomao.zerohack.setting.ConfigManager
import cum.xiaomao.zerohack.setting.GenericConfig
import cum.xiaomao.zerohack.setting.GuiConfig
import cum.xiaomao.zerohack.setting.ModuleConfig
...
internal object Configurations : AbstractModule(
    ...
    init {
        BackgroundScope.launchLooping("Config Auto Saving", 60000L) {
            if (autoSaving && mc.currentScreen !is AbstractZeroGui<*, *> && timer.tickAndReset(savingInterval.toLong())) {
                if (savingFeedBack) NoSpamMessage.sendMessage(this@Configurations, "Auto saving settings...")
                else ZeroHackMod.logger.info("Auto saving settings...")
                ConfigUtils.saveAll()
            }
        }
    ...
    private fun updatePreset(setting: StringSetting, input: String, config: IConfig) {
        ...
        try {
            ...
            ZeroHackMod.logger.warn("Couldn't set path!", e)
        ...
        fun copyPreset(name: String) {
            defaultScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    ZeroHackMod.logger.error("Failed to copy preset", e)
                }
            }
        }
        ...
        fun deletePreset(name: String) {
            defaultScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    ZeroHackMod.logger.error("Failed to delete preset", e)
                }
            }
        }
