...
internal object AutoExcuse : Module(
    ...
    private val file = File("${ZeroHackMod.DIRECTORY}/excuses.txt")
            ...
            loadedExcuses = if (mode == Mode.EXTERNAL) {
                if (file.exists()) {
                    ...
                    try {
                        ...
                        ZeroHackMod.logger.error("Failed loading excuses", e)
                    }
                    cacheList.toTypedArray()
                } else {
                    file.createNewFile()
                    MessageSendUtils.sendNoSpamErrorMessage("$chatName Excuses file is empty!" +
                        ", please add them in the §7excuses.txt§f under the §7.minecraft/ZeroHack§f directory.")
                    defaultExcuses
                }
            } else {
                defaultExcuses
