...
import cum.xiaomao.zerohack.gui.hudgui.elements.client.Watermark
import cum.xiaomao.zerohack.manager.managers.*
import cum.xiaomao.zerohack.manager.managers.HotbarManager.serverSideItem
...
import cum.xiaomao.zerohack.util.EntityUtils.isPassive
...
import cum.xiaomao.zerohack.util.accessor.*
...
import cum.xiaomao.zerohack.util.combat.CombatUtils.totalHealth
import cum.xiaomao.zerohack.util.combat.CrystalDamage
import cum.xiaomao.zerohack.util.combat.CrystalUtils
import cum.xiaomao.zerohack.util.combat.CrystalUtils.canPlaceCrystalOn
import cum.xiaomao.zerohack.util.combat.CrystalUtils.isResistant
import cum.xiaomao.zerohack.util.combat.ExposureSample
...
import cum.xiaomao.zerohack.util.graphics.*
...
import cum.xiaomao.zerohack.util.items.attackDamage
import cum.xiaomao.zerohack.util.items.duraPercentage
import cum.xiaomao.zerohack.util.math.RotationUtils
import cum.xiaomao.zerohack.util.math.RotationUtils.getRotationTo
import cum.xiaomao.zerohack.util.math.VectorUtils.setAndAdd
import cum.xiaomao.zerohack.util.math.VectorUtils.toViewVec
import cum.xiaomao.zerohack.util.math.vector.*
import cum.xiaomao.zerohack.util.pause.HandPause
import cum.xiaomao.zerohack.util.pause.MainHandPause
import cum.xiaomao.zerohack.util.pause.withPause
import cum.xiaomao.zerohack.util.threads.*
import cum.xiaomao.zerohack.util.world.FastRayTraceAction
import cum.xiaomao.zerohack.util.world.fastRaytrace
import cum.xiaomao.zerohack.util.world.isAir
import cum.xiaomao.zerohack.util.world.rayTraceVisible
...
internal object AutoCrystalPlus : Module(
    name = "AutoCrystal+",
    description = "Lol wtf",
    ...
    // Force Place
    private val forcePlaceHealth by setting("Force Place Health", 8.0f, 0.0f..36.0f, 0.5f, { page == Page.FORCE_PLACE })
    ...
    private val ColorSetting by setting("Color", ColorRGB(255, 255, 255, 220))
    private val syncGui by setting("SyncGui", false, { page == Page.RENDER })
    ...
    init {
        onEnable {
            ...
            ZeroHackMod.logger.trace(Watermark.nameAsString)
        }
    ...
    private inline fun SafeClientEvent.placeDirect(placeInfo: PlaceInfo) {
        ...
        if (hand == null) {
            when (autoSwap) {
                ...
                SwapMode.SWAP -> {
                    ...
                    onMainThread {
                        ...
                        MainHandPause.withPause(AutoCrystalPlus, placeDelay * 2) {
                            swapToSlot(slot)
            ...
            onMainThread {
                HandPause[hand].withPause(AutoCrystalPlus, placeDelay * 2) {
                ...
                prevPos?.let { prevPos ->
                    currentPos?.let { currentPos ->
                        ...
                        if (syncGui)
                            ...
                            renderer.add(box, AutoCrystalPlus.ColorSetting)
